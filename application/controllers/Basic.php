...
class Basic extends MY_Controller {
	...
	public function index()
	{
		...
		$this->load->view('basic/index', compact('logo', 'background', 'footer'));
		return true;
	}
	...
	public function edit_logo()
	{
		...
		if ( empty($_FILES['userfile']['size']) ) { // 無檔案上傳
			$this->load->view('basic/edit_logo', compact('query'));
			return true;
		}
	...
	public function edit_background()
	{
		...
		if ( empty($_FILES['userfile']['size']) ) { // 無檔案上傳
			$this->load->view('basic/edit_background', compact('query'));
			return true;
		}
		@unlink($query->path); // 刪除原有檔案
		$data['path'] = $this->singleUploading()['path'];
	...
	public function edit_footer()
	{
		...
		if ( ! $data = $this->input->post() ) {
			$this->load->view('basic/edit_footer', compact('query'));
			return true;
		}
		$data['id'] = 1;
